...
import { useGetAllCarsQuery, type ICar } from '@/features/car/carApis';

const CompareCar = () => {
  const { data: cars } = useGetAllCarsQuery('');
  const [compareVehicles, { data: comparisonData }] = useCompareVehiclesMutation();

  const [slot1, setSlot1] = useState<ICar | null>(null);
  const [slot2, setSlot2] = useState<ICar | null>(null);

  const handleSelect = (car: ICar) => {
    if (slot1?._id === car._id) return setSlot1(null);
    if (slot2?._id === car._id) return setSlot2(null);
    if (!slot1) return setSlot1(car);
    if (!slot2) return setSlot2(car);
    setSlot2(car);
  };

  const getSlotClass = (car: ICar) => {
    if (slot1?._id === car._id) return 'ring-2 ring-blue-500';
    if (slot2?._id === car._id) return 'ring-2 ring-green-500';
    return 'hover:shadow-md';
  };

  const handleCompare = async () => {
    if (!slot1 || !slot2) return;
    await compareVehicles({ id1: slot1._id, id2: slot2._id, type: 'car' });
  };
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <p className="text-sm text-gray-500 mb-4">
        Click a car to select it. First click = Car 1, second = Car 2.
      </p>
      ...
      <div className="grid grid-cols-4 gap-3">
        {cars?.data?.map((car: ICar) => (
          <div
            ...
            className={`relative border rounded-xl p-3 cursor-pointer transition-all ${getSlotClass(car)}`}
          >
            {slot1?._id === car._id && (
        ...
        <div className="flex-1">
          {slot1 ? (
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-blue-600 font-semibold">Car 1</p>
            ...
            <p className="text-sm text-gray-400 text-center">Select Car 1</p>
          )}
        </div>
          ...
          {slot2 ? (
            <div className="flex items-center gap-3 justify-end">
              <div className="text-right">
                <p className="text-xs text-green-600 font-semibold">Car 2</p>
                <p className="text-sm font-medium">
            ...
            <p className="text-sm text-gray-400 text-center">Select Car 2</p>
          )}
        </div>
      </div>
